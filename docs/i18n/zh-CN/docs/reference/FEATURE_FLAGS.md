...
共 76 个标志，分为 6 个类别。**默认值**是定义中的默认值，即既不存在数据库覆盖值，也不存在环境变量时所使用的值。
...
| `PII_REDACTION_ENABLED`                 | 布尔值 | `false`  | 从请求中修订 PII（独立于 `INPUT_SANITIZER_MODE`）。                                                                                                                                    |
...
| `OUTBOUND_SSRF_GUARD_ENABLED`           | 布尔值 | `true`   | 阻止向私有/内部 IP 范围发送出站请求。                                                                                                                                                  |
| `ALLOW_API_KEY_REVEAL`                  | 布尔值 | `false`  | 允许已经过身份验证的仪表板用户显示已存储的 API 密钥，而不是仅查看掩码值。                                                                                                              |
| `AUTH_LOG_INCLUDE_ACCOUNT_ID`           | 布尔值 | `false`  | 在 AUTH 日志行中包含账户前缀（例如“正在使用 <provider> 账户：abc12345...”）。默认禁用，因此账户标识符会从共享/多租户进程日志中修订。此设置独立于调试模式；切换调试模式不会显示该信息。 |
...
| `AUDIO_REMOTE_PROVIDER_NODES`                   | boolean | `false` |      | 允许 /v1/audio/* 路由使用托管在 localhost 之外、与 OpenAI 兼容的提供者节点。默认关闭——将音频路由到远程主机会改变出口身份，因此必须由运维人员明确决定。始终允许环回节点，且不受此设置影响。                                                                                                                                                |
| `RERANK_REMOTE_PROVIDER_NODES`                  | boolean | `false` |      | 允许 POST /v1/rerank（以及记忆引擎的环回重排序步骤）使用托管在 localhost 之外、与 OpenAI 兼容的提供者节点。默认关闭——路由到远程主机会改变出口身份，因此必须由运维人员明确决定。始终允许环回节点；远程节点还必须通过提供者出站 URL 策略检查。                                                                                              |
...
| `NETWORK_ROTATION_SHARED_EGRESS_GUARD`          | boolean | `true`  |      | 对于多账户轮换执行器，如果发生网络异常（超时、连接被拒绝/重置），且失败账户没有专用代理，则应用短暂冷却，并在当前请求的剩余过程中跳过其他无代理账户，而不是逐一重试。默认开启（安全：不会改变出口 IP，只会降低共享出口账户的延迟/冷却风险）。禁用后，将恢复在第一个无代理账户抛出异常时立即传播该异常。                                   |
| `PROXY_SKIP_RECENTLY_FAILED`                    | boolean | `false` |      | 代理池和 opencode 的按账户轮换会在每个进程的特定时段内停止重新提供刚刚失败的代理（TCP 探测被拒绝，或通过该代理收到 429）；每次重复失败时，该时段都会翻倍，直至达到上限。不会写入代理状态；当所有候选代理都被暂时搁置时，选择结果保持不变。默认关闭。                                                                                      |
| `PROXY_POOL_EGRESS_OBSERVATION`                 | boolean | `false` |      | 在仪表板的代理池下显示过去 24 h 内为其成员提供服务的已观测出口 IP 数量，以及使用这些 IP 的连接数量。此信息为只读，根据代理日志计算，绝不会用于路由。默认关闭。                                                                                                                                                                            |
| `OPENCODE_RESPONSES_STALL_ROTATION`             | boolean | `false` |      | 对于 OpenCode 执行器，监测流式 Responses 回复的第一个正文字节（时间窗口：`RESPONSES_FIRST_BYTE_TIMEOUT_MS`，默认值为 `15000`）。如果 2xx Responses 流在超过该时间窗口后仍无数据，则视为停滞：账户将进入冷却状态，并将请求轮换至下一个账户一次；如果再次停滞，则快速失败。默认关闭：停滞的流会继续按当前行为等待，直至达到流就绪超时时间。 |
| `OPENCODE_USER_BLOCKED_ROTATION`                | boolean | `false` |      | OpenCode 执行器：当收到携带 `user_blocked` 拒绝信息的 403/451（非地理限制，也非 Cloudflare 指纹拒绝）时，将被拒绝的账户置于冷却状态，并且每个请求最多轮换至下一个账户一次；第二次拒绝将按原样返回，且不会标记为成功。默认关闭：绕过上游用户封禁进行路由可能被视为规避措施，并将标记扩散到整个账户池。                                     |
| `OPENCODE_TRANSIENT_FAILOVER_BACKOFF`           | boolean | `false` |      | OpenCode 轮换：连续发生两次上游瞬时故障（5xx 或空响应的 400）后，在切换至下一个账户前暂停——从 1.5 秒开始，此后每次故障将等待时间加倍，单次暂停最长 6 秒，每个请求累计最长 10 秒；客户端断开连接时跳过暂停；等待前会释放失败的响应体。默认关闭：故障转移保持立即执行。                                                                     |
| `OPENCODE_PARK_AND_RESUME`                      | boolean | `false` |      | OpenCode 轮换：在重复出现瞬时 429（或出现新的池压力标记）后，通过心跳机制挂起请求，然后重放一个受限轮次，依次尝试最多 3 个账户，而非向整个账户池并发扩散。默认关闭：每次收到 429 时都与之前完全一样轮换至下一个账户。                                                                                                                     |
| `FLUSH_EMPTY_RETRY_ENABLED`                     | boolean | `false` |      | 对于经过转换的流式轮次，当上游轮次不包含可用内容（仅推理的完成结果或没有任何有价值的数据块）时，在向客户端公开任何内容之前，通过常规凭据路径进行有限次数的重试（最多 `STREAM_RECOVERY.EMPTY_TURN_RETRY_MAX` 次）。默认关闭：空轮次保持当前行为（返回空的 200 或空内容的 502）。                                                           |
| `OPENCODE_RATE_LIMITED_429_EARLY_STOP`          | boolean | `false` |      | OpenCode 轮换：遇到首个被判定为真正速率限制的 429（包含可解析的 `Retry-After`，或响应体中提及速率/用量限制）时停止账户轮换，并原样返回该上游 429。无法分类的 429 将继续轮换。默认关闭：免费层按出口 IP 限制（#9611），因此每次收到 429 时都会轮换，所有账户均尝试失败后将返回最后一个上游 429。                                           |
...
| `OMNIROUTE_ALLOW_LOCAL_PROVIDER_URLS`           | boolean | `true`  |      | 允许添加/验证位于本地/私有地址（127.0.0.1、localhost、LAN）上的提供者。默认启用（本地优先）；如需严格限制为仅允许公共地址，请将其禁用。云元数据地址仍会被阻止。                                                                                                                                                                           |
...
| 键                              | 类型   | 默认值     | 描述                                                                                                                                       |
| ------------------------------- | ------ | ---------- | ------------------------------------------------------------------------------------------------------------------------------------------ |
| `TOOL_POLICY_MODE`              | 枚举   | `disabled` | 工具使用策略的执行模式。可选值：`disabled`、`warn`、`block`。                                                                              |
...
| `CAPABILITY_FILTER_ENABLED`     | 布尔值 | `false`    | 当目标模型缺少所需能力（视觉、工具、结构化输出、上下文窗口）时，在分发前拒绝请求。保护绕过组合层兼容性筛选器、直接发送给单一提供者的请求。 |
| `RADAR_ENABLED`                 | 布尔值 | `false`    | 启用 OmniRoute Radar 模块（目录源页面和同步）。默认关闭；启用后仅解锁 UI，数据同步仍需单独选择启用。                                       |

### 运行时（33）

| 键                                          | 类型    | 默认值  | 重启 | 描述                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------------------------------------------- | ------- | ------- | ---- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `UNIVERSAL_CONTEXT_HANDOFF_ENABLED`         | boolean | `true`  |      | 当组合路由切换模型时，生成并注入对话摘要。禁用后会独立处理模型切换，并阻止针对所有现有及未来组合的后台上下文交接请求。                                                                                                                                                                                                                                                                                                           |
| `RESPONSES_PASSTHROUGH_DROP_COMMENTARY`     | boolean | `true`  |      | 在转发给客户端之前，从 Responses API 直通流中丢弃内部评论阶段的输出项。禁用后可接收原始上游评论。                                                                                                                                                                                                                                                                                                                                |
| `OMNIROUTE_MCP_ENFORCE_SCOPES`              | boolean | `true`  |      | 对 MCP 工具访问强制实施作用域限制。                                                                                                                                                                                                                                                                                                                                                                                              |
...
| `OMNIROUTE_CODEX_APP_SERVER_ENABLED`        | boolean | `true`  |      | 允许 Codex 使用本地 app-server WebSocket JSON-RPC 传输（codexTransport=app-server）。关闭时，已选择使用 app-server 的连接将回退到 Codex 的其他传输方式。                                                                                                                                                                                                                                                                         |
| `OMNIROUTE_EMERGENCY_FALLBACK`              | boolean | `true`  |      | 将预算耗尽的请求路由到免费的紧急回退提供者/模型。（请参阅下方的[紧急预算回退](#emergency-budget-fallback)。）                                                                                                                                                                                                                                                                                                                    |
...
| `STREAM_RECOVERY_TOOLCALL_ORDER_FIX`        | boolean | `false` |      | 确保流中途续传对工具调用安全：一旦已发出工具调用（正在进行中，或已完成且 finish_reason 为 tool_calls），绝不恢复被截断的流；遇到一次空续传后即关闭，而不是耗尽全部预算。关闭时：使用发行版行为。                                                                                                                                                                                                                                 |
| `STREAM_EARLY_EOF_SIBLING_FAILOVER_ENABLED` | boolean | `false` |      | 当 SSE 流在发出任何有用帧之前关闭，且同一连接内有限次数的重试已用尽时，故障转移一次至同级连接；如果没有可用的同级连接，则返回原始的 `STREAM_EARLY_EOF` 502。默认关闭：同一连接重试后，提前 EOF 仍为终止性错误。                                                                                                                                                                                                                  |
| `MODEL_CATALOG_INCLUDE_NAMES`               | boolean | `true`  |      | 在 `/v1/models` 响应中包含便于显示的名称字段。对于仅接受模型 ID 的客户端，请禁用此项。                                                                                                                                                                                                                                                                                                                                           |
| `MODELS_CATALOG_PREFIX_MODE`                | enum    | `dual`  |      | 控制 `/v1/models` 中模型 ID 的前缀方式。`dual`（默认）会同时生成别名和规范提供者 ID 前缀，以保持向后兼容性。`alias` 仅生成短别名前缀（例如 `ds-web/model`，而非 `deepseek-web/model`）。`canonical` 仅生成完整的提供者 ID 前缀。可选值：`dual`、`alias`、`canonical`。                                                                                                                                                           |
| `ARENA_ELO_SYNC_ENABLED`                    | boolean | `true`  |      | 启用 Arena AI 排行榜 ELO 的定期同步，用于模型智能水平排名。                                                                                                                                                                                                                                                                                                                                                                      |
| `EXPOSE_CC_DISCOVERY_ALIASES`               | boolean | `false` |      | 在 `/v1/models` 上公布 `claude/<provider>/<model>` 镜像 ID，使 Claude Code 网关的模型发现功能可以列出非 Claude 模型。这是三级开关中的全局级别（环境变量优先于控制面板覆盖设置）。请参阅 [Claude Code 配置](../guides/CLAUDE-CODE-CONFIGURATION.md#discovery-aliases--surface-non-claude-models-in-the-model-picker)。                                                                                                            |
| `NO_THINKING_ALIAS_ENABLED`                 | boolean | `true`  |      | `no-think/<provider>/<model>` 网关别名的总开关。开启（默认）：`/v1/models` 会为每个符合条件且支持思考的 Claude 模型公布一个无思考变体，请求中发送的 `no-think/` ID 会解析回实际模型，并抑制推理。关闭：不会公布任何变体，且 `no-think/` ID 会被视为与其他未知模型 ID 相同。启用此项时，每个模型的 `ModelSpec.noThinkingAlias` 选择启用/停用设置仍然适用。                                                                        |
| `OMNIROUTE_DISABLE_THINKING_LEVEL_VARIANTS` | boolean | `false` |      | 禁止在 `/v1/models` 目录中生成思考级别变体（例如 `-low`、`-medium`、`-high`）。                                                                                                                                                                                                                                                                                                                                                  |
| `OMNIROUTE_CHAT_VIRTUAL_LANES`              | boolean | `false` | ✓    | 为提供者分发启用按租户自适应的虚拟准入通道（#9654）：一个租户的突发流量将不再导致另一个租户收到 503。`OMNIROUTE_CHAT_VIRTUAL_LANES` 环境变量优先于此控制面板覆盖设置；更改将在服务器重启后生效。                                                                                                                                                                                                                                 |
| `EXPOSE_FUNCTIONAL_GATEWAY_MIRRORS`         | boolean | `false` |      | 对于规范所有者没有有效凭据、但由具有有效凭据的透传网关进行路由的模型，在 /v1/models 中公布 <gateway-alias>/<model> 镜像 ID。警告：全局启用时，会为所有客户端添加目录条目。                                                                                                                                                                                                                                                       |
| `NEWAPI_AGGREGATOR_BALANCE`                 | boolean | `false` |      | 为与 New-API / One-API / Sub2API 聚合器兼容的节点启用余额检测。启用后，已设置聚合器标志的兼容节点将在仪表板和配额预检路由中报告其余额。                                                                                                                                                                                                                                                                                          |
| `SERVER_OWNED_TOOL_LOOP_ENABLED`            | boolean | `false` |      | 继续执行服务器所有的非流式工具调用，直到模型返回客户端可用的响应。                                                                                                                                                                                                                                                                                                                                                               |
| `SEARCH_STATS_HIDE_DELETED_CONNECTIONS`     | boolean | `false` |      | 搜索统计和最近搜索仅统计仍具有活动连接的提供者（无密钥提供者（如 duckduckgo-free）始终计入）。关闭时，将保留每一条带有提供者 ID 的搜索记录。                                                                                                                                                                                                                                                                                     |
| `FREE_BADGE_REQUIRES_PROVIDER_FREE_TIER`    | boolean | `false` |      | 仪表板提供者页面：仅在提供者明确支持的信号上显示“免费”徽章——不再采用显示名称启发式规则，并忽略没有已记录免费层级的已注册提供者上的非布尔型免费字段和 :free 后缀。关闭时，保留历史徽章规则。                                                                                                                                                                                                                                      |
| `RETRY_AFTER_PROVENANCE_ENABLED`            | boolean | `false` |      | 对于聚合后的 429/503 不可用响应，当无法确定具体的未来重试时间时，省略 `Retry-After`（而不是使用虚构的 1 秒），添加 `error.retry_after_provenance`（`signal` \| `none`），并允许组合排空路径从 JSON 和纯文本上游响应正文中读取文字形式的重试提示。该字段仅出现在由 `unavailableResponse()` 构建的响应中；其他 429/503 响应正文保持不变。                                                                                          |
| `PROTECTED_PRIORITY_INFRA_502_ENABLED`      | boolean | `false` |      | 当标记为“仅在配额耗尽时回退”的 `priority` 组合目标因可证明并非配额问题的原因（提供者断路器已打开、预测延迟跳过）而停止组合时，返回 502，而不是看似配额问题的 503。因锁定、冷却、不可用、耗尽和并发上限导致的停止仍返回 503。                                                                                                                                                                                                     |
| `MISTRAL_AMBIGUOUS_401_SOFT_LOCKOUT`        | boolean | `false` |      | 对于已吊销的密钥和配额已耗尽的情况，单纯的 Mistral 401（`{"detail":"Unauthorized"}`，无明确的身份验证信号）完全相同。启用后，系统会将连接置于冷却状态，而不是将其标记为 `expired`；每个连接每小时最多 3 次，下一次则会将其标记为 `expired`，因此已吊销的密钥最终仍会进入该状态。默认关闭：与之前一样，每个单纯的 Mistral 401 都会将连接标记为 `expired`。                                                                        |
| `XAI_OAUTH_LIVE_MODEL_DISCOVERY`            | boolean | `false` |      | 对于 `xai-oauth` 连接，使用 OAuth bearer token 从 `https://api.x.ai/v1/models` 获取实时 xAI 模型目录，而不是使用冻结的静态种子。默认关闭：`xai-oauth` 将继续原样提供静态种子。若解析过程中出现任何错误，发现机制将回退到种子（尚未验证 x.ai 是否接受在此端点使用 OAuth bearer）。                                                                                                                                                |
| `BATCH_AND_FILE_AUTO_CLEANUP_ENABLED`       | boolean | `false` |      | 允许自动清理任务删除早于 `OMNIROUTE_BATCH_RETENTION_DAYS` 的终态（已完成/失败/已取消/已过期）Batch API 作业及其逐行检查点，并清除超过其自身 `expires_at` 的已上传文件的 BLOB 内容。默认关闭：在操作员主动启用之前，所有现有安装都会完全按之前的方式保留这些数据。无论是否启用，由操作员触发的 `DELETE /api/v1/batches/delete-completed` 路由均不受影响——它是一项独立且无条件的公共 API 契约。                                    |
| `ANTIGRAVITY_ACCOUNT_LEASE_ENABLED`         | boolean | `false` |      | 在选中 Antigravity 账户的请求的流式处理生命周期内保留该账户，从而防止并发重试或凭据移交重新选中已分配给进行中流的账户。保留范围限定为（连接、可调用的上游模型），因此一个账户仍可同时为两个不同模型提供服务。当该模型的所有合格账户均已被租用时，请求将返回结构化的 503 `antigravity_pool_busy` 响应，并附带有上限的 `Retry-After`，而不是继续将请求堆积到繁忙账户上。默认关闭：账户选择方式与之前完全相同，且不会进行任何保留。 |
...
| `SKILLS_SANDBOX_NETWORK_ENABLED`          | boolean | `false` | 启用技能沙箱环境中的网络访问。                                                                                                                                                              |
| `PROXY_HEALTH_BLOCKED_RESETS_STREAK`      | boolean | `false` | 在代理健康检查扫描中，当探测被目标拒绝（401/403/429）时，重置代理的连续失败次数。默认关闭：拒绝保持中性状态（#10654）。无论如何，5xx 均保持结果不确定；拒绝绝不会移除、禁用或重新激活代理。 |
| `DB_HEALTHCHECK_STARTUP_DEFERRED_ENABLED` | boolean | `false` | 在服务器开始接受请求后（通过 `setImmediate`）运行启动时的数据库完整性/健康检查，而不是阻塞启动直至检查完成（#13717）。默认关闭：启动过程与此 PR 之前完全相同，仍会被阻塞。                  |
...
> 读取的普通环境变量，而非功能标志：它们没有数据库覆盖值，也没有仪表板开关。请参阅
> [`ENVIRONMENT.md`](./ENVIRONMENT.md#4-security--authentication)。

> [!NOTE]
> `Restart` 列用于标记带有 `requiresRestart: true` 的标志——其值会
> 立即持久化，但只有在进程重新加载后才会生效。枚举
> 标志会拒绝其允许集合之外的任何值（在服务器端的
