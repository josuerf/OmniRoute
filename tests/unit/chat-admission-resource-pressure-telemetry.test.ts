...
import { resourcePressureRejectionResponse, resourcePressureRetryAfterSeconds } from "../../src/shared/middleware/chatAdmissionResponses.ts";
import {
  describeResourcePressureShedDetail,
...
test("resourcePressureRetryAfterSeconds: floors, ramps, and caps", () => {
  assert.equal(resourcePressureRetryAfterSeconds(0), 2, "fresh critical stays at the pre-existing floor");
  assert.equal(resourcePressureRetryAfterSeconds(-500), 2, "negative input clamps to the floor");
  assert.equal(resourcePressureRetryAfterSeconds(15_000), 9, "halfway through the ramp (2 + 0.5*13 = 8.5, rounds to 9)");
  assert.equal(resourcePressureRetryAfterSeconds(30_000), 15, "ramp ceiling at RESOURCE_PRESSURE_RETRY_AFTER_RAMP_MS");
  assert.equal(resourcePressureRetryAfterSeconds(120_000), 15, "never exceeds the ceiling, however long critical persists");
});
